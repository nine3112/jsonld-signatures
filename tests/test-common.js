    ...
    context(suiteName + ' w/promise API', () => {
      it('should sign a document w/security context', async () => {
        ...
        const signed = await jsigs.sign(testDoc, {
          ...
          suite: new Suite(mock.parameters.sign[suiteName]),
          purpose: new NoOpProofPurpose()
        });
        let expected = mock.securityContextSigned[suiteName];
        if(suiteName === 'RsaSignature2018') {
          // RSA-PSS is pseudorandom, so ignore actual signature value
      ...
      it('should verify a document w/security context', async () => {
        ...
        const result = await jsigs.verify(signed, {
          documentLoader: testLoader,
          suite: new Suite(mock.parameters.verify[suiteName]),
          purpose: new NoOpProofPurpose()
        });
        const expected = {
          verified: true,
      ...
      it('should verify a document w/o security context', async () => {
        ...
        const expected = {
          ...
          results: [{
            proof: {
              '@context': constants.SECURITY_CONTEXT_URL,
              ...mock.securityContextSigned[suiteName].proof
            },
            verified: true
          }]
        };
      ...
      it('should verify a document w/security context w/passed key',
        ...
        const result = await jsigs.verify(signed, {
          documentLoader: testLoader,
          suite: new Suite(mock.parameters.verifyWithPassedKey[suiteName]),
          purpose: new NoOpProofPurpose()
        });
        const expected = {
          verified: true,
      ...
      it.skip('should sign a document with multiple signatures', async () => {
      });

      it.skip('should verify a document with multiple set signatures', async () => {
      });
    });
